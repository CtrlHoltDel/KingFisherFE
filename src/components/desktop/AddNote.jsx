    ...
    <div className="notes__add-note">
      <div className="notes__add-note__type-toggle">
        <button
          style={{
            ...buttonStyle,
            opacity: noteType === NOTE_TYPE ? "100%" : "60%",
          }}
          onClick={() => handleChangeType(NOTE_TYPE)}
        >
          NOTE
        </button>
        <button
          style={{
            ...buttonStyle,
            opacity: noteType === TENDENCY_TYPE ? "100%" : "60%",
          }}
          onClick={() => handleChangeType(TENDENCY_TYPE)}
        >
          TENDENCY
        </button>
      </div>
      <form onSubmit={handleFormSubmit}>
        ...
        />
        <button
          className="notes__add-note__add-button"
          style={buttonStyle}
          disabled={!noteInput}
        >
          <p>
            Add {noteType} for {player.name.length > 20 ? `${player.name.slice(0, 20)}..` : player.name}
          </p>
        </button>
      </form>
    </div>
